...
def get_face_dataloaders(
    ...
    if class_names != FER2013_CLASSES:
        raise ValueError(
            f"FER2013 class order mismatch. Found {class_names}, "
            f"expected {FER2013_CLASSES}. Rename folders or update the "
            "canonical class order deliberately."
        )

    if test_dataset.classes != FER2013_CLASSES:
        raise ValueError(
            f"FER2013 test class order mismatch. Found {test_dataset.classes}, "
