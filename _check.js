...
const Policy = { consent:{ text_processing:true, microphone:false, camera:false, session_memory:false }, locale:"en-IN" };
...
- SHORT — 1 to 3 sentences. This is speech, not an essay. No bullet points, no lists, no emoji spam, no headings.
...
/* ── VOICE ──────────────────────────────────────────────────────────────
   ...
   Browser speechSynthesis stays as the offline fallback. */
let playCtx=null,playAnalyser=null,playSrc=null,lipBuf=null;
...
async function say(text,persist,gesture,mood){
  ...
  try{
    await speakGemini(text,id,gesture,mood);
  }catch(e){
    console.warn("Gemini TTS unavailable, falling back:",e.message);
    speakBrowser(text,id,gesture);
  }
}
...
async function speakGemini(text,id,gesture,mood){
  const ctx=audioCtxReady();
  const r=await fetch("/api/tts",
    {method:"POST",headers:{"Content-Type":"application/json"},body:JSON.stringify({text,mood})});
  ...
  playSrc=ctx.createBufferSource(); playSrc.buffer=buf;
  playAnalyser=ctx.createAnalyser(); playAnalyser.fftSize=1024;
  lipBuf=new Float32Array(playAnalyser.fftSize);
  playSrc.connect(playAnalyser); playAnalyser.connect(ctx.destination);

  playGesture(gesture);
  speech.on=true; speech.mode="audio"; speechStartedAt=performance.now();
  const finish=()=>{ if(id!==speakId) return;
    speech.on=false; speech.mode=null; mouthTarget=0; playAnalyser=null; };
  playSrc.onended=finish;
  playSrc.start();
  // if the context was blocked by autoplay policy, onended may never fire —
  // never let that strand speech.on=true
  setTimeout(()=>{ if(id===speakId&&speech.on) finish(); },(buf.duration+2)*1000);
}

function speakBrowser(text,id,gesture){
  if(!("speechSynthesis"in window)) return;
  speechSynthesis.cancel();
  const u=new SpeechSynthesisUtterance(text);
  u.lang="hi-IN"; u.rate=.97; u.pitch=1.12;
  const vs=speechSynthesis.getVoices();
  const v=vs.find(x=>/neerja|heera|female|zira|samantha/i.test(x.name))||vs.find(x=>/^(hi|en)/i.test(x.lang));
  if(v) u.voice=v;
  playGesture(gesture);
  speech.on=true; speech.mode="synth"; speech.text=text; speech.at=0; speech.len=0;
  speech.t0=performance.now(); speech.mark=performance.now(); speechStartedAt=performance.now();
  u.onboundary=e=>{ if(id!==speakId) return;
    speech.at=e.charIndex; speech.len=e.charLength||0; speech.mark=performance.now(); };
  const done=()=>{ if(id!==speakId) return;
    speech.on=false; speech.mode=null; mouthTarget=0; };
  u.onend=done; u.onerror=done;
  speechSynthesis.speak(u);
  setTimeout(()=>{ if(id===speakId&&speech.on) done(); },Math.max(2500,text.length*72)+4000);
}
...
async function boot(){
  ...
  setEmotion("neutral");
  setTimeout(()=>runTurn(null),600);   // her opening line is generated, not scripted
}
function fit(){
...
function grabFrame(maxW=512){
  ...
  try{
    const b64=shot.toDataURL("image/jpeg",.72).split(",")[1];
...
async function geminiReply(msg,a,risk){
  ...
  const body={
    ...
    contents,
    generationConfig:{temperature:1.1,maxOutputTokens:800,thinkingConfig:{thinkingLevel:"low"}}
  };
...
async function tryAmbientGlance(){
  ...
  try{
    const body={
      ...
      generationConfig:{temperature:1,maxOutputTokens:200,thinkingConfig:{thinkingLevel:"low"}}
    };
...
async function runTurn(msg){
  ...
  thinking(true);
  let text;
  try{ text=await geminiReply(msg,a,risk); }
  catch(e){
    console.error(e);
    text=risk.state==="no_evidence"
      ? "Sorry yaar, mera connection thoda lag kar gaya. Phir se bol na?"
      : "Main yahin hoon tere saath. "+RES;
  }
  ...
  thinking(false);
  say(enforceSafety(text,risk),risk.state!=="no_evidence",gesture,mood);
}
...
function recOn(){
  ...
  recWanted=true;
  if(!rec){
    rec=new SR(); rec.lang=Policy.locale; rec.continuous=true; rec.interimResults=false;
    rec.onresult=e=>{
